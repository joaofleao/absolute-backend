...
export const getUserWatches = query({
  ...
  handler: async (ctx, args): Promise<any> => {
    ...
    for (const movie of args.movies) {
      const myWatch = await ctx.runQuery(api.oscar_movies.getUserWatch, {
        movieId: movie,
      })
...
export const getFriendsWatches = query({
  ...
  handler: async (ctx, args): Promise<any> => {
    ...
    const friendsData = await ctx.runQuery(api.oscar_movies.getFriendsData)

    for (const movie of args.movies) {
      const friendsWatch = await ctx.runQuery(api.oscar_movies.getFriendsWatch, {
        movieId: movie,
      })
