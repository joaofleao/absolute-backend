...
import { authErrorCatalog, AuthErrorCode } from './errorCatalog'
...
function mapRetrieveAccountError(error: unknown): never {
  ...
  throw new ConvexError('auth-generic')
}
...
function handleUnexpectedError(error: unknown): never {
  ...
  if (message.includes('Invalid password')) {
    throw new ConvexError('password-weak')
  }
  throw new ConvexError('auth-generic')
}
  ...
  return ConvexCredentials<DataModel>({
    ...
    authorize: async (params, ctx) => {
      try {
        ...
        if (flow === 'signUp') {
          if (secret === undefined) {
            throw new ConvexError('password-missing-signup')
          }
          ...
          if (secret === undefined) {
            throw new ConvexError('password-missing-signin')
          }
          let retrieved: {
          ...
          if (!config.reset) {
            throw new ConvexError('reset-not-enabled')
          }
          let retrievedAccount: GenericDoc<DataModel, 'authAccounts'>
          try {
            ;({ account: retrievedAccount } = await retrieveAccount(ctx, {
              provider,
              account: { id: email },
            }))
          } catch (error) {
          ...
          return await signInViaProvider(ctx, config.reset, {
            accountId: retrievedAccount._id,
            params,
          })
          ...
          if (params.newPassword === undefined) {
            throw new ConvexError('new-password-missing')
          }
          ...
          if (!config.verify) {
            throw new ConvexError('verify-not-enabled')
          }
          let retrievedAccount: GenericDoc<DataModel, 'authAccounts'>
          try {
            ;({ account: retrievedAccount } = await retrieveAccount(ctx, {
          ...
          throw new ConvexError('flow-invalid')
        }
        // START: Optional, email verification during sign in
